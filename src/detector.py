...
class PotholeDetector:
    ...
    def _detect_with_heuristic(self, frame: np.ndarray) -> List[Detection]:
        ...
        # Enhance local contrast to improve shadow/wet boundary visibility.
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        enhanced = clahe.apply(blur)

        # Dark-region mask (typical dry potholes).
        dark = cv2.adaptiveThreshold(
            enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 21, 5
        )

        # Reflective/wet potholes are often low saturation + high value.
        mask = dark
        if self.use_reflective:
            hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
            h, s, v = cv2.split(hsv)
            reflective = cv2.inRange(s, 0, int(self.sat_thresh)) & cv2.inRange(v, int(self.val_thresh), 255)
            mask = cv2.bitwise_or(mask, reflective)

        # Texture mask to reject flat areas (puddles still have texture on edges).
        lap = cv2.Laplacian(enhanced, cv2.CV_64F)
        tex = np.uint8(np.clip(np.abs(lap) * 2, 0, 255))
        _, tex_mask = cv2.threshold(tex, self.texture_thresh, 255, cv2.THRESH_BINARY)
        ...
        for cnt in contours:
            ...
            if aspect < 0.15 or aspect > 6.0:
                continue
            conf = min(0.95, area / (frame_area * 0.05))
            dets.append(Detection(label="pothole", confidence=conf, bbox=(x, y, x + cw, y + ch)))
        return dets
